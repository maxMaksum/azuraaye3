...
fun FaceListScreen(
    ...
    showEditPhotoCapture?.let { face ->
        ...
        val galleryLauncher = rememberLauncherForActivityResult(ActivityResultContracts.GetContent()) { uri ->
            uri?.let {
                val bmp = com.example.crashcourse.utils.PhotoStorageUtils.loadBitmapFromUri(context, it)
                if (bmp != null) {
                    scope.launch {
                        val result = com.example.crashcourse.utils.PhotoProcessingUtils.processBitmapForFaceEmbedding(context, bmp)
                        if (result != null) {
            ...
            Box(
                ...
                Column(Modifier.fillMaxSize()) {
                    if (photoEditMode == null) {
                        ...
                        com.example.crashcourse.ui.add.FaceCaptureScreen(
